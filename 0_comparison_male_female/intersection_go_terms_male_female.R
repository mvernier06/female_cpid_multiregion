...
fgsea_male <- "cpid_multiregion/data/2__differential_expression_analysis/fgsea_obj.Rdata"
...
ggplot(intersection_df, aes(x=timepoint, y=n_terms, color=region)) +
  ...
  labs(title="FGSEA terms (padj < 0.05) overlap between male and female",
...
### Meme chose en prennant pval = 0.1 
rm(list=ls())
fgsea_01_female <- "female_cpid_multiregion/data/2__differential_expression_analysis/fgsea_01_obj.Rdata"
fgsea_01_male <- "cpid_multiregion/data/2__differential_expression_analysis/fgsea_01_obj.Rdata"
output_data_path <- "female_cpid_multiregion/data/0_comparison_male_female/"
output_path <- "female_cpid_multiregion/graphs_results/0_comparison_male_female/"

female_env <- new.env()
load(fgsea_01_female, envir = female_env)
female_objs <- ls(female_env, pattern = "^fgsea_01_")
for (obj in female_objs) {
  assign(paste0(obj, "_female"), female_env[[obj]])
}

male_env <- new.env()
load(fgsea_01_male, envir = male_env)
male_objs <- ls(male_env, pattern = "^fgsea_01_")
for (obj in male_objs) {
  assign(paste0(obj, "_male"), male_env[[obj]])
}

# Homogénéiser les noms 
fgsea_01_NAc_1_female <- fgsea_01_Nac_1_female
fgsea_01_NAc_2_female <- fgsea_01_Nac_2_female
fgsea_01_NAc_3_female <- fgsea_01_Nac_3_female
...
get_terms_description <- function(fgsea, pval_threshold = 0.05, padj_threshold = 0.1,   NES_threshold = 0){
  fgsea@result %>%
    filter(pvalue < pval_threshold & p.adjust < padj_threshold & abs(NES) > NES_threshold) %>%
    pull(Description)
}
...
for(tp in timepoints){
  for(reg in regions){
    ...
    male_name <- paste0("fgsea_01_", reg, "_", tp, "_male")
    female_name <- paste0("fgsea_01_", reg, "_", tp, "_female")
...
intersection_sizes <- sapply(intersections_list, length)
intersection_sizes
save(intersections_list, file=paste0(output_data_path, "intersection_terms/intersection_list_fgsea_01_terms.Rdata"))
...
ggplot(intersection_df, aes(x=timepoint, y=n_terms, color=region)) +
  geom_point(aes(pch=region)) +
  geom_line(aes(group=region)) +
  labs(title="FGSEA terms (padj < 0.1) overlap between male and female",
       y = "Number of overlapping terms",
       color = "Region", shape = "Region") +
  theme_bw() +
  theme(axis.title.x=element_blank())
ggsave(plot=last_plot(), paste0(output_path, "intersection_terms/overlaping_terms_01_male_female.png"))
