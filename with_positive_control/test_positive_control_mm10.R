...
#### PATHS ####
raw_counts.path <- "female_cpid_multiregion/with_positive_control/mm10/data/cpid_multireg_control_counts.txt"
coldata.path <- "female_cpid_multiregion/with_positive_control/mm10/data/coldata_control.ods"
output.path <- "female_cpid_multiregion/with_positive_control/mm10/data/2__differential_expression_analysis/CPID_sham_vs_cuff_betaprior.csv" # file to save results
...
common_samples <- intersect(colnames(raw_counts), rownames(coldata))
...
#counts.path <- "female_cpid_multiregion/with_positive_control/mm10/data/cpid_multireg_control_counts.txt"
annot_table.path <- "female_cpid_multiregion/data/count_data/annotation_final.csv"
output.path <- "female_cpid_multiregion/with_positive_control/mm10/data/2__differential_expression_analysis/annotated_counts.rds" # where to save results

# Annotate counts
counts <- raw_counts
counts <-  rownames_to_column(counts)
counts$rowname <-  str_replace(counts$rowname, "\\..*", "") 

ens2symbol <- read.csv(annot_table.path, sep=";")
ens2symbol <- ens2symbol %>% dplyr::select(!X)

annotated_counts <- inner_join(counts, ens2symbol, by=c("rowname"="Gene.stable.ID"))
annotated_counts <- annotated_counts %>% relocate(MGI.symbol, .before=rowname) 
...
output.path <- "female_cpid_multiregion/with_positive_control/mm10/data/2__differential_expression_analysis/annotated_counts.csv"
...
test <-"female_cpid_multiregion/with_positive_control/mm10/data/2__differential_expression_analysis/annotated_counts.csv"
...
counts_test <- read_csv(test)
counts_test <- counts_test %>% select(-rowname) 
