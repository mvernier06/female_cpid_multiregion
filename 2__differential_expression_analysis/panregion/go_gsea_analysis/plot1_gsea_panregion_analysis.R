...
plot.file <- "graphs_results/panregion/go_gsea_analysis/gsea/"
...
get_enrichplot <- function(region, tp, term, plot.path) { 
  ...
  fgsea_res <- get(paste0("fgsea_panregion_1_", tp), envir = .GlobalEnv) # changer 01 ou 1 en fonction du filtre voulu
  ...
  ggsave(file= paste0(region_dir, term, "_panregion_fgsea_1_tp", tp, ".png"), plot=plot, 
...
heatmap_files <- function(tp_list) {
  ...
  assign(paste0("gsea_panregion_01_alltp"), gsea_alltp, envir = .GlobalEnv)
}
...
go_heatmap <- function(terms_list, plot.path, top10 = FALSE, max_line_length = 30) {
  for (term in terms_list) {
    ...
    obj_name <- paste0("gsea_panregion_01_alltp")
