...
## pareil en mettant pvalueCutoff 0.1 (pour tester les overlap de terms : script intersection_go_terms_male_female.R )
set.seed(123)
for(reg in regionList){
  for(tp in tpList){
    message("Doing GSEA analysis of ", reg, " for TP", tp)
    ...
    assign(paste("fgsea_01", reg, tp, sep = "_"), gse)
  }
}

## SAVE ##
rm(gse)
gse_obj <- ls()[grepl("fgsea_01_",ls())]
save(list=gse_obj, file= paste0(output.path, "fgsea_01_obj.Rdata"))


## COUNT RESULT ##
for(reg in regionList){
  for(tp in tpList){
    gse_obj <- paste("fgsea_01", reg, tp, sep="_")
    print(paste0(reg, " tp", tp, ": ",nrow(get(gse_obj))))
  }
}
