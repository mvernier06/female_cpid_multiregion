...
reg <- "Ins"
print(paste0("Plotting network of MEGENA modules for region: ", reg))
...
cell_types_enrichment <- paste0("data/4__MEGENA/MEGENA_male_female_insula/with_chr_X_Y/enrichment_cell_types/enrichment_cell_types_", reg, ".Rdata")
genes_chrXY.path <- "data/4__MEGENA/MEGENA_male_female_insula/with_chr_X_Y/genes_chrXY_MGI_symbols.txt"
enrichment_chrXY.path <- paste0("data/4__MEGENA/MEGENA_male_female_insula/with_chr_X_Y/enrichment_chrXY/enrichment_chrXY_", reg, ".Rdata")
...
### Presence de gènes chrX et Y dans les modules ###
setwd(project_dir)
...
network_plot_chrXY_presence <- function(){
  ...
  p <- ggraph(flareGraph, 'igraph', algorithm = 'tree', circular = TRUE) + 
    ...
    scale_color_identity() +
    ggforce::theme_no_axes() +
    ggtitle(paste0("Modules containing chrX/Y genes - ", reg))
  setwd(presence_chrXY_plots.path)

  ggsave(p, filename=paste0("chrXY_presence_", reg, ".png"))
  ...
  p_dendrogram <- ggraph(flareGraph, 'igraph', algorithm = 'tree', circular = FALSE) + 
   ...
   ggsave(p_dendrogram, filename=paste0("dendrogram_chrXY_presence_", reg, ".png"))
}

init_network()
network_plot_chrXY_presence()

### Enrichment en gènes chrX et Y dans les modules ###
setwd(project_dir)
load(enrichment_chrXY.path)
t <- Overlap_chrXY %>%
  rownames_to_column("modules") %>%
  dplyr::select(modules, overlap.pval_chrXY) %>%
  rename(pval = overlap.pval_chrXY) %>%
  mutate(signif = case_when(pval < 0.05 ~ "enriched",
                            TRUE ~ NA))
...
modules_enriched_chrXY <- t$modules[which(t$signif == "enriched")]

network_plot_chrXY_enrichment <- function(){

  nodes$color <- NA
  nodes$color[nodes$name %in% modules_enriched_chrXY] <- "red"
  nodes$color[is.na(nodes$color)] <- "grey80"
  
  nodes$label <- ifelse(nodes$name %in% modules_enriched_chrXY, nodes$name, NA)
  ...
  p <- ggraph(flareGraph, 'igraph', algorithm = 'tree', circular = TRUE) + 
    ...
    scale_color_identity() +
    ggforce::theme_no_axes() +
    ggtitle(paste0("Modules enriched in chrX/Y genes - ", reg))
  setwd(enrichment_chrXY_plots.path)

  ggsave(p, filename=paste0("chrXY_enrichment_", reg, ".png"))
  ...
  p_dendrogram <- ggraph(flareGraph, 'igraph', algorithm = 'tree', circular = FALSE) + 
   ...
   ggsave(p_dendrogram, filename=paste0("dendrogram_chrXY_enrichment_", reg, ".png"))
}
